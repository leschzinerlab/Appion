...
def setupParserOptions():
        ...
        parser.add_option("--appion",dest="appion",type="string",metavar="STRING",
                help="Appion basename to remove (E.g. Remove '14jul22y_' from '14jul22y_Jul15_18.09.54.mrc'")
        parser.add_option("--path",dest="path",type="string",metavar="STRING",
                help="Path to appion micrographs")
        parser.add_option("--output",dest="output",type="string", metavar="STRING",
                help="Output path for symbolically linked, newly named micrographs")
...
def renameBoxFiles(params):
	...
	boxlist = glob.glob('%s/*.mrc' %(params['path']))
	...
	for box in boxlist:
		...
		os.symlink('%s' %(box),'%s/%s' %(params['output'],newbox))
