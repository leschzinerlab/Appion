...
class ctfEstimateLoop(appionLoop2.AppionLoop):
	...
	def processImage(self, imgdata):
		...
		if self.params['cs_override'] >= 0.1:	
			inputparams = {
		...
		if self.params['cs_override'] == 0:
                        inputparams = {
                                'orig': os.path.join(imgdata['session']['image path'], imgdata['filename']+".mrc"),
                                'input': apDisplay.short(imgdata['filename'])+".mrc",
                                'output': apDisplay.short(imgdata['filename'])+"-pow.mrc",
                                'cs': self.params['cs_override'],
                                'kv': imgdata['scope']['high tension']/1000.0,
                                'ampcnst': bestampcontrast,
                                'xmag': xmag,
                                'dstep': dstep*1e6,
	...
	def setupParserOptions(self):
		...
		self.parser.add_option("--cs", dest="cs_override", type="float", default=100.0,
                        help="Cs value used for CTF estimation; overrides any Cs value already in Appion", metavar="#")
		## true/false
